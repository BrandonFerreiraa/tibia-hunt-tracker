...
**Fast-follow (Story 4.1, v0.5):** a TibiaData API não envia headers de CORS — o design original de buscar criatura direto do browser (`fetch()`) nunca funcionaria em produção. Só descoberto no teste manual do usuário. Corrigido migrando a busca para uma RPC server-side (`get_creature`, migration `018_get_creature_rpc.sql`), seguindo o mesmo padrão já usado pela verificação de personagem (Epic 2, `sync_character_profile`).

**Validado end-to-end em produção (2026-08-28):** com as migrations `016`, `017` e `018` aplicadas, o usuário logou e o fluxo foi verificado ao vivo: grid de cards em Dashboard e Hunts Compartilhadas, mini-ícones reais (demon, grimeleech, hellflayer, vexclaw), fallback correto pra monstro não encontrado ("ferumbras"), e o modal de detalhes funcionando tanto pra hunt própria quanto pra hunt de OUTRO usuário ("War do Rift", de Brandon Testeira) — confirmando que a correção de RLS da migration 017 funciona de verdade. Todas as stories fecharam com verdict QA **PASS**.
