-- UX tweak: when the character isn't found in the highscore scan, keep track of WHICH
-- category was checked (only null out value/rank), so the UI can show "fora do top 1000
-- em Magic Level" instead of just showing nothing with no context.
...
begin
  ...
  if v_found is null then
    ...
    return jsonb_build_object('success', true, 'found', false, 'category', p_category);
  end if;
