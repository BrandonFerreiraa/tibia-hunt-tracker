-- Fix: http_get() can raise a hard exception (timeout, "HTTP request cancelled", DNS
-- failure) instead of just returning a response with a bad status code. The functions
-- only checked response.status, so a raised exception bubbled up as a raw Postgres
-- error instead of our clean api_unavailable taxonomy. Wrap every http_get call in
-- BEGIN/EXCEPTION.
