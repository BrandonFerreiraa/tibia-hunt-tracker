-- Fix: linhas de `creatures` já cacheadas ANTES da migration 019 (com
-- synced_at fresco) nunca disparavam a busca na wiki, porque o guard de
-- "já está fresco" no topo de get_creature() retornava o cache antigo sem
-- nunca chegar no bloco que tenta a TibiaWiki — os *_pct ficavam null pra
-- sempre até synced_at expirar (30 dias). Mesmo problema já visto com
-- display_name (migration 018) quando a coluna foi adicionada depois.
--
-- Fix: `wiki_checked_at` marca toda TENTATIVA de busca na wiki (sucesso ou
-- não), separado de `wiki_synced_at` (só sucesso). O guard de "fresco" agora
-- também exige `wiki_checked_at is not null` — uma linha antiga (sem essa
-- coluna preenchida) sempre passa por pelo menos uma tentativa de busca na
-- wiki antes de voltar a ser servida 100% do cache. Depois da primeira
-- tentativa (mesmo sem achar nada), não fica reconsultando a wiki à toa.

alter table creatures add column if not exists wiki_checked_at timestamptz;
